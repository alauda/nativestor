...
func convertConfig(dev *topolvmv2.DeviceClass) (*cluster.DeviceClass, error) {
		...
		if ele.Type == Loop {
			...
			if ele.Auto {
				...
				if !created {
					...
					s := topolvmv2.LoopState{Name: ele.Name, Status: cluster.LoopCreateSuccessful, DeviceName: ele.Name}
